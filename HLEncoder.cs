...
namespace ScreenLogicConnect
{
    public class HLEncoder
    {
        ...
        private readonly byte[] sm_rcon = new byte[] { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a, 0x2f, 0x5e, 0xbc, 0x63, 0xc6, 0x97, 0x35, 0x6a, 0xd4, 0xb3, 0x7d, 0xfa, 0xef, 0xc5, 0x91 };
        readonly uint[] tk = new uint[MAX_KC];
        ...
        public Span<byte> GetEncryptedPassword(string cls)
        {
        ...
        private void MakeKeyFromBlock(ReadOnlySpan<byte> block)
        {
            if (block == null)
            {
                return;
            ...
            if (block.Length == 16 || block.Length == 24 || block.Length == 32)
            {
                ...
                {
                    tk[blockChunk] = (uint)((block[(blockChunk * 4) + 0] << 24) | (block[(blockChunk * 4) + 1] << 16) | (block[(blockChunk * 4) + 2] << 8) | block[(blockChunk * 4) + 3]);
                }
                ...
                {
                    {
                        ...
                        tk[0] ^= (uint)(((((sm_S[(tkVal >> 16) & 0xFF] << 24) ^ (sm_S[(tkVal >> 8) & 0xFF] << 16)) ^ (sm_S[tkVal & 0xFF] << 8)) ^ sm_S[(tkVal >> 24) & 0xFF]) ^ (sm_rcon[rconPointer] << 24));
                    }
                    ...
                    else
                    {
                        ...
                        tk[blockChunkMidpoint] ^= (uint)(((sm_S[tkVal & 0xFF] ^ (sm_S[(tkVal >> 8) & 0xFF] << 8)) ^ (sm_S[(tkVal >> 16) & 0xFF] << 16)) ^ (sm_S[(tkVal >> 24) & 0xFF] << 24));
        ...
        private Span<byte> EncryptBlock(ReadOnlySpan<byte> block)
        {
            if (!bKeyInit)
            {
                return null;
        ...
        Span<byte> Encrypt(string sString)
        {
            if (bKeyInit)
            {
        ...
        Span<byte> EncryptData(ReadOnlySpan<byte> data)
        {
            ...
            {
                var encryptedBlock = EncryptBlock(data.Slice(iBlock * BLOCK_SIZE, BLOCK_SIZE));
